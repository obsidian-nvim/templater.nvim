...
local get_line = function(str, line_num)
...
local pos_to_line = function(str, pos)
	local line = 1
...
Compiler.render = function(self)
	return table.concat(self.buffer)
end

Compiler.push = function(self, str, ...)
	local i = self.i + 1
	self.buffer[i] = str
	self.i = i
	if ... then
...
Compiler.header = function(self)
	return self:push("local _tostring, _b, _b_i = ...\n")
end

Compiler.footer = function(self)
	return self:push("return _b")
end

Compiler.increment = function(self)
	return self:push("_b_i = _b_i + 1\n")
end

Compiler.mark = function(self, pos)
	return self:push("--[[", tostring(pos), "]] ")
end

Compiler.assign = function(self, ...)
	self:push("_b[_b_i] = ", ...)
	if ... then
		return self:push("\n")
	end
...
Compiler.compile_chunks = function(self, chunks)
	self:header()
	for i = 1, #chunks do
...
Parser.in_string = function(self, start, stop)
...
Parser.push_raw = function(self, start, stop)
	return insert(self.chunks, self.str:sub(start, stop))
end

Parser.push_code = function(self, kind, start, stop)
...
Parser.compile = function(self, str)
...
Parser.parse_error = function(self, err, code)
...
Parser.error_for_pos = function(self, source_pos, err_msg)
...
Parser.load = function(self, code, name)
	if name == nil then
		name = "etlua"
	end
	local code_fn
...
Parser.run = function(fn, env, buffer, i, ...)
	if env == nil then
		env = {}
	end
	...
	return fn(tostring, buffer, i, ...)
end

Parser.compile_to_lua = function(self, str, ...)
	local success, err = self:parse(str)
	if not success then
		return nil, err
	end
...
Parser.chunks_to_lua = function(self, compiler_cls)
